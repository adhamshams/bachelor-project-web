            ...
            <div style={{display: 'flex', flexDirection: 'row', marginLeft: 50, width: window.innerWidth-200, marginRight: 120, height: 600, marginBottom: -15}}>  
                <div style={{display: 'flex', flexDirection: 'column', overflowY: 'scroll', marginTop: 20, height: 600}}>
                    {chats.map((chat, index) => (
                        <ChatCard onClick={() => setSelectedChat(chat)} lastText={chat.messages[chat.messages.length-1]} chat={chat} key={index}/>
                    ))} 
                </div> 
                <div style={{display: 'flex', flexDirection: 'column', flex: 1, marginTop: 20, height: 600}}>
                    ...
                    <div style={{flex: 1, display: 'flex', flexDirection: 'column'}}>
                        ...
                        <div style={{height: 500, display: 'flex', flexDirection: 'column', overflowY: 'scroll'}}>
                            {selectedChat.messages.map((message, index) => (
                                <div key={index} style={{backgroundColor: message.author.id === user.uid ? '#CE1212' : '#eeebdd', borderRadius: 20, maxWidth: 200, display: 'flex', marginTop: 5, marginLeft: message.author.id === user.uid ? "auto" : 10, marginRight: 10}}>
    ...
    <Container style={{display: "flex", flexDirection: 'column'}}>
        ...
        <div style={{display: 'flex', flexDirection: 'row', width: '100%'}}>
          <div style={{display: 'flex', flexDirection: 'column', marginLeft: 80, width: 200}}>
            ...
            <label onMouseEnter={() => setHover5('#810000')} onMouseLeave={() => setHover5('#CE1212')} style={{fontFamily: 'Archivo', cursor: 'pointer', marginTop: 10, fontSize: 15, color: hover5}} onClick={() => history.push('/profile/messages')}>Messages</label>
