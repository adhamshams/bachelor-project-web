...
function RedBorderButton(props) {
  ...
  return (
    ...
    >
      {props.loading ? <ReactLoading type={"spin"} color={"#f4f4f4"} height={27} width={27}/> : <label style={{fontFamily: 'Archivo', fontSize: 17, color: hover === '#810000' ? '#eeebdd' : '#CE1212' , cursor: props.disabled ? 'default': 'pointer'}}>{props.title}</label>}
    </div>
  );
}
