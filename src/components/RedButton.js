...
function RedButton(props) {
  ...
  return (
    ...
    >
      {props.loading ? <ReactLoading type={"spin"} color={"#eeebdd"} height={27} width={27}/> : <label style={{fontFamily: 'Archivo', fontSize: props.style.fontSize||17, color: props.style.color||'#eeebdd', cursor: props.disabled? 'default': 'pointer'}}>{props.title}</label>}
    </div>
  );
}
